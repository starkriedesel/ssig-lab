class Flag < ActiveRecord::Base
  ...
  def self.generate_flag!(user_id, challenge_id)
    flag = Flag.where(:user_id => user_id, :challenge_id => challenge_id).first
    flag ||= Flag.create({user_id: user_id, challenge_id: challenge_id, value: generate_flag, nonce: generate_nonce})
    flag
  end
end
